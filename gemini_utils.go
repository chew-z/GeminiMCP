...
func (s *GeminiServer) formatResponse(resp *genai.GenerateContentResponse) *internalCallToolResponse {
	var content string
	thinking := ""
...
// formatMCPResponse formats the Gemini API response directly to MCP response format
func (s *GeminiServer) formatMCPResponse(resp *genai.GenerateContentResponse) *mcp.CallToolResult {
	var content string
	thinking := ""
	...
	if thinking != "" {
		...
		if err == nil {
			return &mcp.CallToolResult{
				Content: []mcp.Content{
					mcp.NewTextContent(string(thinkingJSON)),
				},
			}
		}
		// Fall back to just content if JSON conversion fails
