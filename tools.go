...
var GeminiAskTool = mcp.NewTool(
	"gemini_ask",
	mcp.WithDescription(
		"gemini_ask sends a prompt to the configured LLM provider, optionally enriched with GitHub repository context "+
			"(files, PRs, commits, diffs). All github_* parameters are independent and combinable. "+
			"Returns the model's answer as Markdown text."),
	...
	mcp.WithArray(
		...
		mcp.Description(
			"Optional: array of file paths in github_repo at github_ref to attach as inline file context, "+
				"e.g. [\"main.go\", \"tools.go\"]. Independent of and combinable with github_pr / github_commits / github_diff_*.",
		),
		mcp.WithStringItems(),
	),
	mcp.WithNumber("github_pr", mcp.Description("Optional: pull request number in github_repo.")),
	mcp.WithArray("github_commits", mcp.Description("Optional: array of commit SHAs (short or full), e.g. [\"a1b2c3d\"]."), mcp.WithStringItems()),
	...
	// The schema is strict: with server-side input validation enabled
	// (WithInputSchemaValidation + WithStrictInputSchemaDefault), unknown
	// arguments such as the removed model controls (model, thinking_level)
	// are rejected as SEP-1303 tool-execution errors. The handler still
	// ignores them defensively on direct-invocation paths.
