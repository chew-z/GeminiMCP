...
var GeminiAskTool = mcp.NewTool(
	...
	mcp.WithArray("file_paths", mcp.Description("Optional: An array of local file paths. This is only supported in 'stdio' transport mode. For 'http' transport, you must use 'github_files'.")),
	...
	mcp.WithArray("github_files", mcp.Description("Optional: An array of file paths from a GitHub repository. This is required for providing files in 'http' transport mode. Requires 'github_repo' to be set.")),
